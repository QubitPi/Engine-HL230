...
RUN mkdir /engine-hl230
COPY . /engine-hl230
RUN cd /engine-hl230 && mvn clean package -Dmaven.test.skip=true
...
COPY --from=build /engine-hl230/target/engine-hl230-$WS_VERSION.war $JETTY_WEBAPPS_DIR/ROOT.war
